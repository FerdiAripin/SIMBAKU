...
class LapBarangKeluarController extends Controller
{
    ...
    public function pdf(Request $request)
    {
        try {
            // Log semua parameter
            \Log::info('PDF Barang Keluar Request - Start Processing', [
                'all_params' => $request->all(),
                'method' => $request->method(),
                'url' => $request->fullUrl()
            ]);

            // Step 1: Validasi parameter jenis
            ...
            // Step 2: Build query dengan step-by-step logging
            \Log::info('Building database query...');

            $query = BarangkeluarModel::select([
                'tbl_barangkeluar.*',
            ...
            $query->orderBy('tbl_barangkeluar.bk_id', 'DESC');
            ...
            $data = [
                'data' => $results,
                'title' => 'Laporan Barang Keluar - ' . ucfirst($jenis),
                'tglawal' => $request->get('tglawal'),
                'tglakhir' => $request->get('tglakhir'),
                'jenis' => $jenis,
                'kategori_nama' => null
            ...
            try {
                ...
                $data['web'] = $webConfig ?: (object)[
                    ...
                    'web_deskripsi' => 'Laporan Barang Keluar'
                ];
            ...
            $viewName = 'Admin.Laporan.BarangKeluar.pdf';
            ...
            $filename = "laporan-barang-keluar-{$jenis}-{$timestamp}.pdf";

            if ($request->filled('tglawal') && $request->filled('tglakhir')) {
                $tglawal_clean = str_replace(['/', '-', ' '], '_', $request->get('tglawal'));
                $tglakhir_clean = str_replace(['/', '-', ' '], '_', $request->get('tglakhir'));
                $filename = "laporan-barang-keluar-{$jenis}-{$tglawal_clean}_to_{$tglakhir_clean}-{$timestamp}.pdf";
            }
    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->rawColumns(['tgl', 'keterangan', 'barang', 'kategori'])->make(true);
        }
    }
}
