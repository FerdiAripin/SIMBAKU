...
class ProdukBaruController extends Controller
{
    public function index()
    {
        ...
        $data["hakTambah"] = AksesModel::leftJoin('tbl_submenu', 'tbl_submenu.submenu_id', '=', 'tbl_akses.submenu_id')->where(array('tbl_akses.role_id' => Session::get('user')->role_id, 'tbl_submenu.submenu_judul' => 'Produk Baru', 'tbl_akses.akses_type' => 'create'))->count();
        $data["kategori"] =  KategoriModel::orderBy('kategori_id', 'DESC')->get();
        return view('Admin.ProdukBaru.index', $data);
    }

    public function detail($id)
    {
        $data["title"] = "Produk Baru";
        $data["barang"] = BarangModel::where('barang_type', '=', 'baru')
    ...
    {
        $data = BarangModel::where('barang_type', '=', 'baru')->leftJoin('tbl_kategori', 'tbl_kategori.kategori_id', '=', 'tbl_barang.kategori_id')->where('tbl_barang.barang_kode', '=', $id)->get();
    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = BarangModel::where('barang_type', '=', 'baru')->leftJoin('tbl_kategori', 'tbl_kategori.kategori_id', '=', 'tbl_barang.kategori_id')->orderBy('barang_id', 'DESC')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $button = '';
                    $hakEdit = AksesModel::leftJoin('tbl_submenu', 'tbl_submenu.submenu_id', '=', 'tbl_akses.submenu_id')->where(array('tbl_akses.role_id' => Session::get('user')->role_id, 'tbl_submenu.submenu_judul' => 'Produk Baru', 'tbl_akses.akses_type' => 'update'))->count();
                    $hakDelete = AksesModel::leftJoin('tbl_submenu', 'tbl_submenu.submenu_id', '=', 'tbl_akses.submenu_id')->where(array('tbl_akses.role_id' => Session::get('user')->role_id, 'tbl_submenu.submenu_judul' => 'Produk Baru', 'tbl_akses.akses_type' => 'delete'))->count();
                    $button = '<div class="g-2">';

                    $button .= '<a class="btn modal-effect text-primary btn-sm" href="' . url('/admin/produk_baru/detail/' . $row->barang_id) . '"><span class="fe fe-eye text-primary fs-14"></span></a>';
