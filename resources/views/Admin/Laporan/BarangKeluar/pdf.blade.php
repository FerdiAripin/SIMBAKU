...
<body>
    ...
    <div class="report-title" style="text-align: center; align-items: center">Laporan Produk Keluar</div>
    ...
    @if (isset($data) && count($data) > 0)
        <table id="main-table">
            ...
            <tbody>
                ...
                @foreach ($data as $row)
                    @php
                        $totalJumlah += isset($row->bk_jumlah) ? $row->bk_jumlah : 0;
                    @endphp
                    <tr>
                        <td class="text-center">{{ $no++ }}</td>
                        <td class="text-center">
                            @if (isset($row->bk_tanggal) && $row->bk_tanggal)
                                {{ date('d/m/Y', strtotime($row->bk_tanggal)) }}
                            @else
                                -
                            @endif
                        </td>
                        <td class="text-center">{{ isset($row->bk_kode) ? $row->bk_kode : '-' }}</td>
                        ...
                        <td class="text-center">
                            {{ isset($row->bk_jumlah) ? number_format($row->bk_jumlah, 0, ',', '.') : '0' }}</td>
                        <td class="text-center">{{ isset($row->bk_keterangan) ? $row->bk_keterangan : '-' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="summary">
            <table>
                ...
                <tr>
                    <td class="label">Total Produk Keluar</td>
                    <td class="colon">:</td>
    ...
    @else
        <div class="no-data">
            ...
            <p>Tidak ada data produk keluar untuk filter yang dipilih</p>
        </div>
    @endif
</body>
